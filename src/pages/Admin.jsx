...
const STATUS_STYLES = {
  ...
  featured:  { label: 'Featured',  className: 'status-done' },
...
export default function Admin() {
  ...
  async function loadAll() {
    ...
    const [
      profilesRes, logsRes, logsAllRes,
      requestsRes, prosRes
    ] = await Promise.all([
    ...
    const newThisWeek   = allProfiles.filter(p => p.created_at >= weekAgo).length;
    const pendingReqs   = (requestsRes.data ?? []).filter(r => r.status === 'pending').length;
  ...
  async function addProfessional(e) {
    ...
    const { data, error } = await supabase.from('professionals').insert({
      ...
      topmate_url: proForm.topmate_url || null,
      linkedin_url:proForm.linkedin_url || null,
      bio:         proForm.bio || null,
      tags,
      sort_order:  pros.length + 1,
    }).select().single();
    setSavingPro(false);
    ...
    <div className="min-h-screen bg-mesh p-4 md:p-8" style={{ color: 'var(--c-text-1)' }}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard icon={<BarChart2 size={20} />}  value={stats.problemsThisWeek}   label="Problems This Week"   color="#f59e0b" />
          <StatCard icon={<Clock size={20} />}      value={stats.pendingRequests}    label="Pending Requests"     color="#ec4899" />
        </div>

        {/* Tabs */}
        <div className="flex gap-2 mb-6 flex-wrap">
          {[
            { id: 'overview',  label: 'Users', icon: Users },
            { id: 'requests',  label: 'Feature Requests', icon: FileText },
        ...
        {tab === 'requests' && (
          <div className="space-y-4 fade-in">
            {requests.length === 0 ? (
              <div className="glass rounded-2xl p-10 text-center text-slate-500">No requests yet.</div>
            ) : requests.map(req => {
                  ...
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div className="min-w-0">
                      <div className="flex items-center gap-3 mb-1 flex-wrap">
                        ...
                        <span className={`badge text-xs ${s.className ?? ''}`} style={s.style ?? {}}>
                          {s.label}
                        </span>
                      </div>
                      <p className="text-xs font-mono text-slate-400">{req.email} · {req.role} @ {req.company}</p>
                      {req.topmate_url && <p className="text-xs text-indigo-400 mt-1">{req.topmate_url}</p>}
                      {req.message && <p className="text-sm text-slate-300 mt-2 leading-relaxed">{req.message}</p>}
                      <p className="text-xs text-slate-600 mt-2 font-mono">{format(new Date(req.created_at), 'MMM d, yyyy HH:mm')}</p>
                    </div>
                    <div className="flex flex-wrap gap-2 shrink-0">
                      {req.status !== 'contacted' && (
                        <button onClick={() => updateRequestStatus(req.id, 'contacted')} className="btn-ghost text-xs px-3 py-1.5">
                          Mark Contacted
                        </button>
                      )}
                      {req.status !== 'featured' && (
                        <button onClick={() => updateRequestStatus(req.id, 'featured')} className="btn-emerald text-xs px-3 py-1.5">
                          <CheckCircle2 size={12} className="inline mr-1" />Featured
                        </button>
                      )}
                      {req.status !== 'rejected' && (
                        <button
        ...
        {tab === 'professionals' && (
          <div className="space-y-4 fade-in">
            ...
            {/* Professionals list */}
            {pros.length === 0 ? (
              <div className="glass rounded-2xl p-10 text-center text-slate-500">No professionals yet. Add one above.</div>
            ) : pros.map(pro => (
              <div key={pro.id} className="glass rounded-2xl p-5 flex items-start justify-between gap-4 flex-wrap">
                <div className="min-w-0">
                  ...
                  {pro.topmate_url && <p className="text-xs text-indigo-400 mt-1">{pro.topmate_url}</p>}
                  {pro.tags?.length > 0 && (
                ...
                <div className="flex gap-2 shrink-0">
                  <button
                    onClick={() => togglePro(pro.id, pro.is_active)}
