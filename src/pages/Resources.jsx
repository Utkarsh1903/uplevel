...
import { RESOURCE_CATEGORIES, RESOURCE_TYPES, FEATURED_VIDEOS } from '../lib/resources';
...
export default function Resources() {
  ...
  const displayedVideos = onlyMust
    ? FEATURED_VIDEOS.filter(v => v.mustKnow)
    : FEATURED_VIDEOS;
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 fade-in">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Curated, 100% free resources and video tutorials. No noise — only the ones that actually help.</p>
      </div>
      ...
      <div className="flex gap-2">
        {[
          ...
          { id: 'videos',    label: '▶️ Video Library' },
        ].map(t => (
          <button
            key={t.id}
      ...
      {mainTab === 'videos' && (
        <div>
          <p className="text-slate-400 text-sm mb-4 font-mono">
            // {displayedVideos.length} curated video tutorials · click ▶ to watch in-app
          </p>
          {displayedVideos.length === 0 ? (
            <div className="text-center py-12 text-slate-500">No videos match filter.</div>
          ) : (
